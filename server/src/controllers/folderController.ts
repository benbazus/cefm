...
export const createNewFolder = async (
  ...
  try {
    ...
    const folder = await folderService.createNewFolder(
      userId,
      folderName,
      parentFolderId,
      userInfo.ipAddress,
...
export const copyFolder = async (
  ...
  try {
    ...
    const copyFolder = async (folder: any, parentId: string | null) => {
      ...
      for (const child of folder.children) {
        await copyFolder(child, newFolder.id);
      }

      return newFolder;
    };

    const copiedFolder = await copyFolder(sourceFolder, newParentId);
