...
const getFilePath = (fileName: string, fileId: string) => `./uploads/file-${fileId}-${fileName}`;
...
export const uploadStatus = async (req: Request, res: Response, next: NextFunction): Promise<void> => {

    if (req.query && req.query.fileName && req.query.fileId) {
        getFileDetails(getFilePath(req?.query?.fileName as string, req.query.fileId as string))
            .then((stats) => {
