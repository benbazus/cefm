...
import { File, FileActivity, Folder, Prisma, StorageHistory } from '@prisma/client';
import path from 'path';
import bcrypt from 'bcrypt';
import { promises as fs } from 'fs';
import md5 from 'md5';
import { config } from '../config/config';
import { decodeFolder, encodeFolderId, generateUniqueId } from '../utils/helpers';
...
export const uploadFile = async (userId: string, folderId: string | undefined, file: Express.Multer.File): Promise<File> => {
...
export const saveFileData = async (originalname: string, folderId: string, mimetype: string, size: number, filePath: string, userId: string): Promise<File> => {
    try {
        const uploadedFile = await prisma.file.create({
            data: {
                name: originalname,
...
export const getFilesActivities = async (userId: string, folderId?: string): Promise<FileActivity[]> => {
    try {
        const whereClause = {
            userId,
            ...folderId && { folderId },
...
export const getFileActivities = async (userId: string, fileId: string): Promise<FileActivity[]> => {
    try {
...
export const getStorageHistories = async (userId: string): Promise<StorageHistory[]> => {
    try {
...
export const getStorageHistory = async (userId: string): Promise<StorageHistory | null> => {
    try {
...
export const getFiles = async (userId: string, folderId?: string): Promise<File[]> => {
    try {
...
export const checkPassword = async (password: string, itemId: string): Promise<string> => {
    try {
...
export async function deleteFile(fileId: string, userId: string): Promise<string> {
    try {
        ...
        const filePath = path.join(process.env.FILE_STORAGE_PATH!, file.filePath as string);
...
export const getDocuments = async (userId: string): Promise<{ files: File[] }> => {
...
export const getCustomDocuments = async (userId: string): Promise<{ documents: Document[] }> => {
...
export const getShared = async (userId: string): Promise<{ folders: Folder[]; files: File[] }> => {
    try {
...
export const getTrashed = async (userId: string): Promise<{ folders: Folder[]; files: File[]; documents: Document[] }> => {
    try {
        const folders = await prisma.folder.findMany({
...
export const getExcelFiles = async (userId: string): Promise<{ files: File[] }> => {
    try {
        const files = await prisma.file.findMany({
            where: {
...
export const getAudioFiles = async (userId: string): Promise<{ files: File[] }> => {
    try {
        const files = await prisma.file.findMany({
            where: {
                fileType: 'Audio File',
...
export const getPPFiles = async (userId: string): Promise<{ files: File[] }> => {
    try {
        const files = await prisma.file.findMany({
            where: {
...
export const getVideoFiles = async (userId: string): Promise<{ files: File[] }> => {
    try {
        const files = await prisma.file.findMany({
            where: {
                fileType: 'Video File',
...
export const getPDFFiles = async (userId: string): Promise<{ files: File[] }> => {
    try {
        const files = await prisma.file.findMany({
            where: {
                fileType: 'Adobe Portable Document Format (PDF)',
...
export const getWordFiles = async (userId: string): Promise<{ files: File[] }> => {
    try {
        const files = await prisma.file.findMany({
            where: {
...
export const renameFile = async (fileId: string, userId: string, newName: string): Promise<string> => {
    try {
        const file = await prisma.file.findUnique({
            where: { id: fileId },
            include: { folder: true }
...
export const restoreFile = async (id: string, type: 'file' | 'folder'): Promise<string> => {
    try {
        if (type === 'folder') {
...
export const moveToTrash = async (id: string, userId: string): Promise<string> => {
    try {
        const file = await prisma.file.findUnique({ where: { id } });
...
export const shareLink = async (userId: string, itemId: string,): Promise<{ url: string }> => {

    let sharedFileUrl: string | null = null;

    const existingFile = await prisma.file.findUnique({ where: { id: itemId } });
    const existingFolder = await prisma.folder.findUnique({ where: { id: itemId } });
