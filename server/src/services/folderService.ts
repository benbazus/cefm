...
export const createNewFolder1 = async (
    ...
    try {
        ...
        const baseFolder = path.join(process.cwd(), 'public', 'File Manager', user?.email as string)
        let newFolderPath: string
        let newFolderUrl: string
        let location: string
        let finalParentFolderId = parentFolderId
        ...
        if (!finalParentFolderId) {
            const rootFolder = await prisma.folder.findFirst({
                where: { name: user.email as string, userId: user.id },
            })
            ...
            finalParentFolderId = rootFolder.id
            newFolderPath = path.join(baseFolder, folderName)
            newFolderUrl = `${process.env.PUBLIC_APP_URL}/File Manager/${encodeURIComponent(user?.email as string)}/${encodeURIComponent(folderName)}`
            location = `/${folderName}`
        } else {
            const parentFolder = await prisma.folder.findUnique({
                where: { id: finalParentFolderId },
            ...
            newFolderUrl = parentFolder.folderUrl
                ? `${parentFolder.folderUrl}/${encodeURIComponent(folderName)}`
                : `${process.env.PUBLIC_APP_URL}/File Manager/${encodeURIComponent(user?.email as string)}/${encodeURIComponent(folderName)}`
...
export const getRootFolder = async (userId: string): Promise<Folder> => {
    const folder = await prisma.folder.findFirst({ where: { parentId: null, userId }, });
    return folder as Folder;
}

// // In your folder service file (e.g., folderService.ts)
// export const getFolderChildren = async (parentId: string) => {
//     try {
...
export const getFoldersAndFilesByFolderId = async (userId: string, parentId: string): Promise<{ folders: Folder[]; files: File[]; documents: Document[] }> => {
    try {
...
export const getRootChildren = async (userId: string): Promise<{ folders: Folder[]; files: File[]; documents: Document[] }> => {
    try {
        ...
        const rootFolder = await prisma.folder.findFirst({
            where: { name: user?.email as string, userId, parentId: null, trashed: false, }
        });
...
export const createFolder = async (userId: string, parentId: string | undefined, name: string): Promise<Folder> => {
...
export const findOrCreateFolder = async (userId: string, parentId: string | undefined, name: string): Promise<Folder> => {
...
export const renameFolder = async (id: string, userId: string, newName: string): Promise<string> => {
...
export const getFolderFileCount = async (folderId: string): Promise<FolderFileInfo> => {
    try {
...
// In your folder service file (e.g., folderService.ts)
export const getFolderFileCount1 = async (folderId: string): Promise<number> => {
    try {
...
export const getFolderDetails = async (folderId: string): Promise<FolderDetails> => {
    ...
    const checkUser = await prisma.user.findUnique({ where: { id: folder.userId }, });
