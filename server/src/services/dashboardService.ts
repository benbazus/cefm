...
export const recentActivity = async (userId: string): Promise<RecentActivity[]> => {
    try {
        ...
        return recentActivities.map((activity): RecentActivity => ({
            id: activity.id,
            fileName: activity.File?.name || activity.Folder?.name || 'Unknown File',
...
export const storageInfo = async (userId: string): Promise<StorageInfo | null> => {
...
export const getFileTypeDistribution = async (userId: string): Promise<{ labels: string[]; datasets: any[] }> => {
    ...
    const labels = distribution
        ...
        .filter((fileType): fileType is string => fileType !== null && fileType !== undefined);
