...
export const getRootFolder = async () => {
  try {
    const config = { method: 'GET', url: '/folders/get-root-folder' }
...
export const getCustomDocuments = async () => {
  try {
    const config = { method: 'GET', url: '/files/get-custom-document' }
...
export const getDocuments = async () => {
  try {
    const config = { method: 'GET', url: '/files/get-document' }
...
export const listFiles = async (folderId?: string) => {
  try {
    //const url = folderId ? `/folders/${folderId}` : `/files`

    const url = '/folders/get-root-children'
...
export const copyFile = async (itemId: string) => {
  try {
    const config = {
      method: 'POST',
      url: `/files/${itemId}/copy`,
    }
...
export const getFileVersions = async (versionId: string) => {
  try {
    const config = { method: 'GET', url: `/files/${versionId}/versions` }
...
export const restoreFileVersion = async (fileId: string, versionId: string) => {
  try {
    const config = {
      method: 'POST',
...
export const lockItem = async (itemId: string, isFolder: boolean) => {
  try {
    const endpoint = isFolder ? 'folders' : 'files'

    const config = {
      method: 'POST',
      url: `/${endpoint}/${itemId}/lock`,
    }
...
export const unlockItem = async (itemId: string, isFolder: boolean) => {
  try {
    const endpoint = isFolder ? 'folders' : 'files'

    const config = {
      method: 'POST',
      url: `/${endpoint}/${itemId}/unlock`,
    }
...
): Promise<GetFoldersResponse | null> => {
  try {
    const response = await axiosRequest<GetFoldersResponse>({
      method: 'GET',
      url: `/folders/get-children-folders-folderId/${folderId}`,
...
export const shareDocumentV1 = async (
  documentId: string,
  email: string,
  message: string,
  date: string,
...
export async function uploadChunkService(
  file: File,
  ...
  try {
    const response = await api.post(
      `/files/uploadFiles?${params.toString()}`,
      data,
      { headers }
    )
    return response.data
...
export const shareAFile = async (
  fileId: string,
  expirationDate: string | null,
  password: string | null,
  sharedWith: string,
  shareWithMessage: string | null,
  isPasswordEnabled: boolean,
  isExpirationEnabled: boolean
) => {
  try {
    const response = await api.post(`/files/share-file`, {
      fileId,
      sharedWith,
      password,
      expirationDate,
...
export const filesUploadToServer = async (
  file: File,
  folderId: string | null,
  onProgress: (progress: number) => void
) => {
...
export const checkPassword = async (password: string, fileId: string) => {
  try {
    const response = await publicApi.post('/files/check-password', {
      password,
      fileId,
    })
    return response.data
...
export const downloadItem1 = async (
  isFile: boolean | undefined,
  itemId: string,
  fileName: string
) => {
...
export const deletePermanently = async (isFile: boolean, fileId: string) => {
  try {
    ...
    const response = await api.get(
      `/files/deletePermanently/${fileType}/${fileId}`
    )
    return response.data
  } catch (error) {
    handleError(error)
...
export const downloadFile = async (fileId: string) => {
  const response = await publicApi.get(`/files/${fileId}/download`, {
    responseType: 'blob',
  })
  return response.data
}
...
export const moveItem = async (
  ...
  const response = await api.post(`/${itemType}s/${itemId}/move`, {
    targetFolderId,
  })
  return response.data
}
